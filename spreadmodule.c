...
spread_connect(PyObject *self, PyObject *args, PyObject* kwds)
{
	...
	if (_PyBytes_Resize(
		&group_name,
		strlen(PyBytes_AS_STRING(group_name))) < 0)
	{
		SP_disconnect(_mbox);
		Py_DECREF(mbox);
		return NULL;
...
static struct PyModuleDef _spreadmodule = {
...
PyMODINIT_FUNC PyInit_spread(void)
{
	PyObject *m, *d;
	struct constdef *p;

	/* Create the module and add the functions */
    m = PyModule_Create(&_spreadmodule);
	if (m == NULL)
    {
