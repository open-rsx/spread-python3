...
class SpreadTest(unittest.TestCase):
    ...
    def testMultipleRecipients(self):
        ...
        wr = members[0]
        wr.multicast(spread.FIFO_MESS, group, b"1")
    ...
    def testOneMessageMultipleRecipients(self):
        ...
        wr = members[0]
        wr.multigroup_multicast(spread.FIFO_MESS, member_private_names, b"1")
        ...
        wr.multigroup_multicast(spread.FIFO_MESS, member_private_names, b"2")
    ...
    def testBigMessage(self):
        ...
        try:
            mbox.multicast(spread.SAFE_MESS, group, bytes("X" * size, "ascii"))
    ...
    def testSelfSend(self):
        ...
        group = self._group()
        msgs = [b'aaa', b'bbb', b'ccc', b'quit!']
        start = threading.Lock()
        stop = threading.Lock()
        start.acquire()
        stop.acquire()
        t = threading.Thread(target=self.receiver,
                             args=(mbox, group, msgs, start, stop))
        t.start()
        for msg in msgs:
            start.acquire()
